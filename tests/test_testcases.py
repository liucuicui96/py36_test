...
#获取excel文件的路径
data=LccHandler.excel.read('testcases')

@pytest.mark.parametrize("info",data)
def test_testcases(info,db):
    '''注册用例'''
    ...
    # 数据库访问，获取创建用例之前的用例数
    sql = 'select count(*) from test.tb_testcases'.format(LccHandler.investor_member_id)
    result = LccHandler.db.query(sql)
    before_cases_count = result['count(*)']
    #访问接口
    ...
    # 校验触发接口之后的用例数
    if info["case_id"] == 3:
        sql = 'select count(*) from test.tb_testcases'
        result = db.query(sql)
        after_cases_count = result['count(*)']
        assert before_cases_count+1 == after_cases_count
